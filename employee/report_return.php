...
<TR>
	...
	<TD align="center" bgcolor="#CCCCCC">วันที่รับคืน</TD>
</TR>
    <? 
...
$sqlreserv=mysql_query("select * from   tb_back_detail ,tb_machin where tb_back_detail.mac_id=tb_machin.mac_id and tb_back_detail.b_status='1' and  leas_date  like '%$txt_search%' ");}
	if($showtype=='typedate'){
		$sqlreserv=mysql_query("select * from   tb_back_detail ,tb_machin where tb_back_detail.mac_id=tb_machin.mac_id and tb_back_detail.b_status='1' and  leas_date  like '%$txt_search2%' ");}
		if($showtype=='typerange'){
$sqlreserv=mysql_query("select * from   tb_back_detail ,tb_machin where tb_back_detail.mac_id=tb_machin.mac_id and tb_back_detail.b_status='1' and tb_back_detail.back_date between '$txt_search3' and '$txt_search4'");
}
	else{		$sqlreserv=mysql_query("select * from   tb_back_detail ,tb_machin where tb_back_detail.mac_id=tb_machin.mac_id and tb_back_detail.b_status='1' ORDER BY  `tb_back_detail`.`leas_id` ASC ");
	}
...
echo thai_date1($eng_date2);?> 
		...
		<?$eng_date2=strtotime("$rsreserv[back_date]");    
echo thai_date1($eng_date2);?> 
			</td>
</TR>
<?}?>
